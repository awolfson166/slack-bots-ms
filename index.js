...
Bot.prototype.login = function() {
    this._api('rtm.connect').then(function(data) {
        this.wsUrl = data.url;
        this.self = data.self;
        this.team = data.team;
...
Bot.prototype.postMessage = function(id, text, params) {
    ...
    // console.log("params", params, id, text);
...
Bot.prototype._post = function(type, name, text, params, cb) {
    ...
    return this[method](name).then(function(itemId) {
        return this.postMessage(name, text, params);
    }.bind(this)).always(function(data) {
        // console.log("data", JSON.stringify(data.response_metadata.messages));
        if (cb) {
            cb(data._value);
        }
    });
